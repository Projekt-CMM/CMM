...
/**
 * Controls the right panel of the main GUI. This is a bit more complex, as this panel can have three
 * different states with different user interfaces. The three different states are initialized into an
 * array of JPanels of which is only shown one at a time. The three different states are: <br>
 *  - user is typing code -> right panel shows a "compile" button
 *  	see method "private void initEditMode()" <br>
 *  - compiler returned error messages -> right panel shows message, total errors and a "view error" button
 *  	see method "private void initErrorMode()" <br>
 *  - interpreter is running -> right panel shows tables and lists for variables as well as
 *  	buttons for controlling the interpreter
 *  	see method "private void initRunMode()"
 * 
 * @author fabian
 *
 */
public class GUIrightPanel implements GUIrPanelMod {
	 ...
	 * @param mod Interface for main GUI manipulations
	 */
	...
	//Interfaces of the three modes of the right panel (edit text, compile error, interpreter)
	private final JPanel jRightPanel[];
	
	//Mode of the right panel (edit text, compile error, interpreter)
	private int mode;
	
	//Wrapper class with listeners for the "run" mode. Also contains the debug and I/O stream interface
	...
	//These objects are in an separate panel so that they can be removed during runtime
	private JPanel jPanelVarInfo;
	
	private JLabel jLabel1;
	private JButton jButtonB1;
	
	//Label saying "local variables". Can be edited so that it also shows the function name
	...
	/**
	 * Initializes objects of the "edit" mode, which is active when the user types code
	 * <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT</i><br>
	 * As this is a initialization method, it should generally not be called
	 * outside the constructor.</hr>
	 */
	...
	/**
	 * Initializes the objects of the "error" mode, which is active if there has been a compiler error
	 * <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT</i>.</i><br>
	 * As this is a initialization method, it should generally not be called
	 * outside the constructor.</hr>
	 */
	private void initErrorMode(){
		...
		this.jRightPanel[1].setBorder(BorderFactory.createLineBorder(Color.DARK_GRAY));
		this.jRightPanel[1].setLayout(new BoxLayout(this.jRightPanel[1], BoxLayout.PAGE_AXIS));
	...
	/**
	 * Initializes the objects of the "run" mode, which is active during interpreting
	 * <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT.</i><br>
	 * As this is a initialization method, it should generally not be called
	 * outside the constructor.</hr>
	 */
	private void initRunMode(){
		...
		//Panel layout
		this.jRightPanel[2].setBorder(BorderFactory.createLineBorder(Color.DARK_GRAY));
		this.jRightPanel[2].setLayout(new BoxLayout(this.jRightPanel[2], BoxLayout.PAGE_AXIS));
		...
		this.jPanelVarInfo.setLayout(new BoxLayout(this.jPanelVarInfo, BoxLayout.PAGE_AXIS));
			...
			this.jTableGlobal.getColumn("Value").setCellRenderer(new JTableButtonRenderer(defaultRenderer));
			
			this.jTableGlobal.addMouseListener(new JTableButtonMouseListener(this.jTableGlobal));
			...
			this.jCallStack.setBorder(BorderFactory.createLineBorder(Color.DARK_GRAY));
			this.jCallStack.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);	//Only one item selectable
			...
			this.jTableLocal.setDefaultRenderer(JButton.class, new JTableButtonRenderer(defaultRenderer));
			
			this.jTableLocal.addMouseListener(new JTableButtonMouseListener(this.jTableLocal));
	...
	public void setError(String msg, int line, int col) {
		...
		if( line <= (int)this.modifier.getSourceCodeRegister().get(0)[0] ){
			...
			for( int i = 1; i < register.size(); i++ ){
				if( line >= (int)register.get(i)[0] && line <= (int)register.get(i)[1] ){
		...
		//Error occurred in main file
			...
			this.ErrorPos.setText("..on line " + ExpandSourceCode.correctLine(line, (int)this.modifier.getSourceCodeRegister().get(0)[0], this.modifier.getSourceCodeRegister().size()) + ", col " + col);
		}
	...
	/**
	 * Reads the global variables from the call stack and saves them to the global variables table.
	 * This method is made thread safe as it is regularly called from the interpreter thread.
	 * 
	 * <hr><i>THREAD SAFE, asynchronously invoked. </i>Exception may stop EDT!<hr>
	 */
	public void updateGlobals(){
		...
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				synchronized( globalVarLock ){
					ReadSymbolTable.readGlobals(compileManager, jTableGlobalModel, reference, 
							globalVarBrowser.peek().getName(),
							globalVarBrowser.peek().getType(),
							globalVarBrowser.peek().getAddress());
	...
	/**
	 * Updates the call stack list in the main GUI. Also updates the local variable "int callStackSize",
	 * which is important for function skipping ("step over"). Take care that at least one of the following
	 ...
	 * function skipping ("step over"). Does <b>not</b> update the call stack list of the main GUI.
	 * Take care that at least one of the following methods (all are thread save) is called at
	 ...
	 * </ul>
	 * 
	 * <hr><i>THREAD SAFE by default </i>Exception may stop EDT!<hr>
	 */
	...
	/**
	 * Checks if the function which has to be skipped (as part of the "step over" routine)
	 * is finished. Eventually exits the "step over" mode.
	 * Also updates the local variable "int callStackSize", which is important for
	 * function skipping ("step over"). Does <b>not</b> update the call stack list of the main GUI.
	 * Take care that at least one of the following methods (all are thread save) is called at
	 * each interpreter step
	 * <ul>
	...
	/**
	 * Reads the local variables from the call stack and saves them to the local variables table.
	 * This method is made thread safe as it is regularly called from the interpreter thread.
	 * 
	 * <hr><i>THREAD SAFE, asynchronously invoked. </i>Exception may stop EDT!<hr>
	 */
	public void updateLocals(){
		...
		if( this.localVarBrowser.peek().getType() == StructureContainer.FUNC ){
			java.awt.EventQueue.invokeLater(new Runnable() {
		        public void run() {
		        	synchronized( localVarLock ){
		        		jLabel3.setText( "Local Variables @ " + localVarBrowser.peek().getName() );
		        	}
		        }
		    });
		}
			...
			java.awt.EventQueue.invokeLater(new Runnable() {
		        public void run() {
		        	synchronized( localVarLock ){
		        		jLabel3.setText( localVarBrowser.peek().getName() + " @ " + localVarBrowser.firstElement().getName() );
		        	}
		        }
		    });
		}
		...
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				synchronized( localVarLock ){
				 	ReadSymbolTable.readLocals(compileManager, jTableLocalModel, reference,
							localVarBrowser.peek().getName(),
							localVarBrowser.peek().getType(),
							localVarBrowser.peek().getAddress());
				}	
	...
	/**
	 * Local variable table in the right panel of the main GUI will display the variables of the
	 * given function.
	 * 
	 * <hr><i>THREAD SAFE by default </i><hr>
	 * 
	 * @param name Name of the function
	 * @param index Index of the function (in the call stack; the last function called has the index 0)
	 */
	public void selectFunction( String name, int index ){
		synchronized( localVarLock ){
			this.localVarBrowser.clear();
			this.localVarBrowser.push(new StructureContainer(name, StructureContainer.FUNC, index));
			this.updateLocals();
		}
	}
	
	/**
	 * Sets the variable browsing tree of the local variable table in the right panel of the main GUI
	 * to the current function. This means, the table displays the local variables of the function
	 * which is currently on the top of the call stack.
	 * 
	 * <hr><i>THREAD SAFE by default </i><hr>
	 */
	public void selectFunction(){
		synchronized( localVarLock ){
			...
			this.localVarBrowser.push(new StructureContainer(
					MethodContainer.getMethodName(Memory.loadInt(Memory.getFramePointer()-8)), StructureContainer.FUNC, 0));
		}
	}
	
	/**
	 * Sets one of the variable tables of the right panel of the main GUI to display content of an array
	 * or a structure.
	 * 
	 * <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT</i><hr>
	 * 
	 * @param name Name of the struct/array
	 * @param address Call stack address of the structure
	 * @param type Type of the variable browser entry, see {@link StructureContainer}
	 * @param global TRUE if the struct/array shall be shown in the global variables table,<br>
	 * 					FALSE if the struct/array shall be shown on the local variables table
	 */
	public void selectStruct( String name, int address, int type, boolean global ){
		
		if( global ){
			synchronized( globalVarLock ){
				this.globalVarBrowser.add(new StructureContainer(name, type, address));
			...
			synchronized( localVarLock ){
				this.localVarBrowser.add(new StructureContainer(name, type, address));
	...
	/**
	 * Sets the global variable browser back to the lowest layer (leaves structs and arrays).
	 * Does not update the global variable table, therefore thread safe by default.
	 * 
	 * <hr><i>THREAD SAFE by default </i><hr>
	 */
	public void globalSelectRoot(){
		synchronized( globalVarLock ){
			...
			this.globalVarBrowser.add(new StructureContainer("Global variables", StructureContainer.GLOBAL, Memory.getGlobalPointer()));
		}
	}
	
	/**
	 * Goes one layer back in the [global/local] variable browser (leaves the currently displayed array
	 * or structure). Eventually removes the "back" button (if the browser came to the lowest level).
	 * 
	 * <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT</i><hr>
	 * 
	 * @param global TRUE if command refers to the global variables table,<br>
	 ...
	 * <hr><i>THREAD SAFE by default </i><hr>
	 * 
	 * @return A reference to the right panel's compiler wrapper object, see {@link CMMwrapper}
	 */
	...
	/**
	 * Sets the label for the execution delay in "run" mode to the given value. Adds
	 * "sec" postfix automatically.
	 * 
	 * <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT</i><hr>
	 * 
	...
	/**
	 * Makes the "step over" button at the top of the right panel in the main GUI visible.
	 * 
	 * <hr><i>THREAD SAFE, asynchronously invoked. </i>Exception may stop EDT!<hr>
	 */
	...
	/**
	 * Makes the "step over" button at the top of the right panel in the main GUI visible
	 * and hides the "nest step" button. Used for functions from include files, as these
	 * shall not be visualized step by step.
	 * 
	 * <hr><i>THREAD SAFE, asynchronously invoked. </i>Exception may stop EDT!<hr>
	 */
	...
	/**
	 * Hides the "step over button at the top of the right panel in the main GUI and
	 * makes the "next step" button visible. This is the default state, displayed for
	 * every AST node, except function calls and assignments with calls.
	 * 
	 * <hr><i>THREAD SAFE, asynchronously invoked. </i>Exception may stop EDT!<hr>
	 */
	...
	/**
	 * Initiates stepping over the next function, <b>not</b> the current function.
	 * However, stepping over will abort if function is not entered with the next AST node.
	 * 
	 * <hr><i>THREAD SAFE, calls synchronized function </i><hr>
	 */
	...
	/**
	 * Sets the right panel of the main GUI to the "runtime error" mode, which is a sub-mode of "run" mode.
	 * 
	 * <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT</i><hr>
	 * 
	 * @param message The error message from the interpreter
	 */
	...
	/**
	 * Goes back from "runtime error" mode to "ready" mode. Both are sub-modes of "run" mode
	 * 
	 * <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT</i><hr>
	 */
	 ...
	 * <hr><i>THREAD SAFE by default </i><hr>
	 * 
	 * @return The first line of the original source code, without includes and include code.
	 */
	 ...
	 * 
	 * <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT.</i>
	 * This method is not thread safe as it changes the right panel of the main GUI.<hr>
	 */
	public void compile(){
		...
		try {
			sourceCode = ExpandSourceCode.expand(sourceCode, this.modifier.getWorkingDirectory(), this.modifier.getSourceCodeRegister());
		} catch (IncludeNotFoundException e1) {
			//An include file could not be found
			java.awt.EventQueue.invokeLater(new Runnable() {
		        public void run() {
		        	setError( "Include file not found: \"" + e1.getFileName() + "\"", e1.getLine(), 0 );
		        }
			});
			return;
		}
		...
		this.sourceCodeBeginLine = (int)this.modifier.getSourceCodeRegister().get(0)[0];
		System.out.println("Source code begins @ line " + this.sourceCodeBeginLine + "\n");
	...
	/**
	 * Starts the interpreter thread via the compiler wrapper class.
	 * Initializes the call stack.
	 * 
	 *  <hr><i>NOT THREAD SAFE, do not call from any other thread than EDT</i><hr>
	 */
	public void runInterpreter(){
		this.compileManager.runInterpreter(listener, new IOstream(this.modifier) );
	}
}
