...
public class FileManagerCode {
	 ...
	 */
	public static boolean saveSourceCode( File fileName, String code, String input ){
		...
		File inputFile = new File( fileName.getPath().substring(0, fileName.getPath().indexOf(".cmm")) + ".input.txt" );
		try {
	...
	/**
	 * Opens the given file, generates input file name,
	 * reads the text contained and gives it back as return parameter
	 *  
	 * @param fileName The file which has to be opened
	 * @return The content of the file (as String). Returns null if file reading failed
	 */
