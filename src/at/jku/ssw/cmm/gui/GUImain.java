...
public class GUImain implements GUImainMod {
	...
	/**
	 * Initializes and launches the main GUI and therefore the main part of the program.
	 * <b>This is not the static main function!</b>
	 * Running this method requires calling a constructor with
	 * configuration data before (see above in code).
	 * 
	...
	private void start(){
		...
		this.jFrame.getContentPane().setPreferredSize(new Dimension(this.settings.getSizeX(), this.settings.getSizeY()));
		...
		this.saveDialog = new SaveDialog(this.jFrame, this.jSourcePane, this.settings);
				
		//Initialize the window listener
		this.jFrame.addWindowListener( new WindowEventListener(this.jFrame, this.settings, this.saveDialog) );
		
		//Initialize window component listener
		this.jFrame.addComponentListener( new WindowComponentListener( this.jFrame, this.jSourcePane, this.settings , this.rightPanelControl ) );
		
		//Initialize the source panel listener
		this.jSourcePane.addKeyListener( new SourcePaneKeyListener( (GUIrPanelMod)this.rightPanelControl, this.jSourcePane ));
		
		//Menubar
		InitMenuBar.initFileM(this.jFrame, this.jSourcePane, this.settings, this.rightPanelControl, this.saveDialog);
	...
	private void highlightInputPane(){
		...
		try {                
	        high.addHighlight(0, this.inputHighlightOffset, DefaultHighlighter.DefaultPainter);       
	...
	public void highlightSourceCode(int line, int col) {
		...
		//Correct offset in source code (offset caused by includes)
			...
			line = ExpandSourceCode.correctLine(line, (int)this.codeRegister.get(0)[0], this.codeRegister.size());
