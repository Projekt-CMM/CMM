 ...
 * <ol>
 * <li> This is a listener for all buttons and the slider on the top of the right panel of the main GUI.
 * 		It therefore connects the user's actions with the interpreter. </li>
 * <li> This is why, this class is also the listener for the interpreter debugger interface which is
 * 		responsible for visualizing steps of the program and displaying runtime error messages.
 * </ol>
 * 
 * Special care has to be taken about thread safety, as functions invoked by the interpreter run in a
 * different thread from the awt event dispatcher thread.
 * 
 * @author fabian
 *
 */
public class PanelRunListener implements Debugger {
	...
	/**
	 * This class monitors the user actions on the interpreter runtime buttons (top of right panel) and
	 * executes queries from the interpreter.
	 * 
	 * @param modMain Interface for main GUI manipulations, eg. source code highlighting
	 * @param master Reference to the right panel wrapper class
	 */
	...
	/* --- step over methods --- */
	/**
	 * Initializes the "step over" mode within the right GUI panel. <b>Do not call this method, </b>
	 * for a complete initialization call {@link GUIrightPanel.stepOver()}
	 * 
	 * <hr><i>SYNCHRONIZED | THREAD SAFE </i><hr>
	 */
	...
	public boolean step(Node arg0) {
		...
		if( this.isPauseMode() ){
			...
			case Node.ASSIGN:
				...
				if( arg0.right.kind == Node.CALL ){
					...
					if( arg0.obj.ast != null && arg0.right.obj.ast.line <= this.master.getBeginLine() )
			...
			case Node.CALL:
				//Calling a function from include file -> step over by default
				if( arg0.obj.ast != null && arg0.obj.ast.line <= this.master.getBeginLine() )
					this.master.setStepOverButtonAlone();
				//Calling a local function -> show step over button
		...
		/* --- Node #4: Pause or Run mode --- */
			...
			if( arg0.kind == Node.ASSIGN && arg0.right.kind == Node.CALL ){
				if( arg0.obj.ast != null && arg0.right.obj.ast.line <= this.master.getBeginLine() ){
					System.out.println("Special call: " + arg0.right.kind + ", line 0" + arg0.right.obj.ast.line);
					this.master.stepOver();
				}
			}
			if( arg0.obj.ast != null && arg0.kind == Node.CALL && arg0.obj.ast.line <= this.master.getBeginLine() ){
				this.master.stepOver();
			}
	...
	/**
	 * This method blocks the program until another thread invokes the method {@link userReply()}
	 * <b> Do not call this method </b> unless you know what you do!
	 * 
	 * <hr><i>SYNCHRONIZED | THREAD SAFE </i><hr>
	 */
	...
	/**
	 * This method unblocks the interpreter and enables it to execute the next step.
	 * 
	 * <hr><i>SYNCHRONIZED | THREAD SAFE </i><hr>
	 */
